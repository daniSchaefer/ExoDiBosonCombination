...
for mass in masses:
 ...
 try:
   fWW=open("WWlowmass_cards_13TeV/wwlvj_BulkGraviton"+str(mass)+"_em_HP_unbin.txt").readlines()
 except:
   print "could not open"
   continue
 outfile="WWlowmass_cards_13TeV/wwlvj_BulkGravitonFix"+str(mass)+"_em_HP_unbin.txt"
 print outfile
 f=open(outfile,"w")
 ...
 for l in range(len(fWW)):
  ...
  if "CMS_xww_XS_BulkGraviton_13TeV" in fWW[l]: continue
  if "rate" in fWW[l]:
   line="rate					  "
   ...
   for s in range(len(fWWsplit)):
    ...
    if signal:
     ...
     numberWW=numberWW*BulkWW[mass] # cards from Jennnifer are in units of 0.01 pb
     newline1+="%.3f  "%pdf_
     newline2+="%.3f  "%scale_
    else:
     newline1+="-  "
