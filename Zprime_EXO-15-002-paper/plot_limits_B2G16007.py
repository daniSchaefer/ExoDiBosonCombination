...
def plot_Asympt_limits(label,mainLabel):
 ...
 names = {}
 names["JJLVJZPRIME13"]="lvJ + JJ channels"
 ...
 names["jjww13"]="JJ channel"
 names["lvjww13"]="lvJ channel"
 ...
 for m in mass:
  idx = int((m-800)/100)
  idxA = int((m-600)/50)
  scaleA['JJLVJZPRIME13'][m] = xsecMap13A['CX0(pb)'][idxA]*(xsecMap13A['BRWW'][idxA])
  ...
  if m >= 800:
   ...
   scaleB['JJLVJZPRIME13'][m] = xsecMap13['CX0(pb)'][idx]*(xsecMap13['BRWW'][idx])
   scaleB['ALLZPRIME13'][m] = xsecMap13['CX0(pb)'][idx]*(xsecMap13['BRWW'][idx]+xsecMap13['BRhZ'][idx])
  else:
   scale['JJLVJZPRIME13'][m] = xsecMap13A['CX0(pb)'][idxA]*(xsecMap13A['BRWW'][idxA])
 ...
 for m in mass:
  ...
  #print m,curAsymLimits[3]*scale[mainLabel][m],curAsymLimits[3]
  lastMass = m/1000.
  xbins.append( m/1000. );
 ...
 ytitle = "#sigma_{95%} #times BR_{Z' #rightarrow WW} (pb)"
 ...
 leg2 = ROOT.TLegend(0.40,0.59,0.91,0.79)#(0.3724832,0.5963455,0.8875839,0.7973422)
 ...
 #theoleg = "#sigma_{TH} #times BR_{Z' #rightarrow WW} , HVT_{B}"
 theoleg = "HVT_{B} (g_{V}=3)"
 leg2.AddEntry(curGraph_xs,theoleg,"L")     
 #theoleg = "#sigma_{TH} #times BR_{Z' #rightarrow WW} , HVT_{A}"
 theoleg = "HVT_{A} (g_{V}=1)"
 leg2.AddEntry(curGraph_xsA,theoleg,"L")     

 pt = ROOT.TPaveText(0.40,0.80,0.76,0.86,"NDC")#(0.5486577,0.8355482,0.8993289,0.8920266,"NDC")
 pt.SetTextFont(42)
...
def compare_Asympt_limits(labels,unblind,bands):
 ...
 for c in range(len(canvas)):
  for p in canvas[c].GetListOfPrimitives():
   ...
   #if str(p).find("CrossSectionTheoA") != -1: graphthA.append(p.Clone("graphthA_%s"%labels[c]))
...
if __name__ == '__main__':
 ...
 # 13 TeV LVJ+JJ only
 scenarios["JJLVJZPRIME13TeV"]=["JJLVJZPRIME13","lvjww13","jjww13"]
