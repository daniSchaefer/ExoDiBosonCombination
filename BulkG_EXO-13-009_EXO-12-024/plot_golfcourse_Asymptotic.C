...
void plot_golfcourse_Asymptotic(bool unblind = true, char* width = 0);
void setFPStyle();
...
//   }
  ...
  if (isFullCombination) {
    fr_left = 550.0, fr_down = 8E-4, fr_right = 2950.0, fr_up = 2.0;
  }
  ...
  l1c->SetLineStyle(7);
  l1c->DrawLine(2550.0,fr_down,2550.0,fr_up);
  cMCMC->Update();
  ...
  */
  ...
  if (useNewStyle) {
    if(isFullCombination || isZZChannel) {
      ...
      TPaveText* pave2 = new TPaveText(0.23, 0.18, 0.28, 0.22, "brNDC");
      ...
      TPaveText* pave3 = new TPaveText(0.875, 0.18, 0.925, 0.22, "brNDC");
      pave3->SetFillColor(kWhite);
      pave3->SetTextSize(0.0375);
      pave3->SetTextAlign(11);
      pave3->SetTextFont(22);
  ...
  char fnam[50];
  if (width != 0) {
    sprintf(fnam, "EXOVV_UL_Asymptotic_%s.root", width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVV_UL_Asymptotic_%s.eps", width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVV_UL_Asymptotic_%s.png", width);
    cMCMC->SaveAs(fnam);
    gPad->SetLogy();
    sprintf(fnam, "EXOVV_UL_Asymptotic_%s_log.eps", width);
    cMCMC->SaveAs(fnam);
    sprintf(fnam, "EXOVV_UL_Asymptotic_%s_log.png", width);
    cMCMC->SaveAs(fnam);
  } else {
    cMCMC->SaveAs("EXOVV_UL_Asymptotic.root");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic.eps");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic.pdf");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic.png");
    gPad->SetLogy();
    cMCMC->SaveAs("EXOVV_UL_Asymptotic_log.eps");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic_log.pdf");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic_log.C");
    cMCMC->SaveAs("EXOVV_UL_Asymptotic_log.png");
  }
