...
for mass in masses:
 ...
 HVTWZ={}
 if mass >= 800: HVTWZ[mass]=(xsecMap['CX-(pb)'][m]+xsecMap['CX+(pb)'][m])*xsecMap['BRZW'][m]
 else: HVTWZ[mass]=(xsecMapA['CX-(pb)'][m]+xsecMapA['CX+(pb)'][m])*xsecMapA['BRZW'][m]

 HVTZH={}
 if mass >= 800: HVTZH[mass]=(xsecMap['CX0(pb)'][m])*xsecMap['BRhZ'][m]
 else: HVTZH[mass]=(xsecMapA['CX0(pb)'][m])*xsecMapA['BRhZ'][m]
 ...
 for l in range(len(fZZ)):
  ...
  if "rate" in fZZ[l]:
   ...
   for s in range(len(fZZsplit)):
    ...
    if signal:
     #print numberZZ/bulkZZ[mass]/0.6991/0.10099/19700.
