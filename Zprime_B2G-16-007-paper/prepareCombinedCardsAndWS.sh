...
if [ $MASS -ge 800 ] && [ $MASS -le 2500 ]
    ...
    sed -e 's|CMS_xww_XS_VV|CMS_XS_VV|g' -e 's|CMS_xww_btag_eff|CMS_eff_b|g' -e 's|CMS_xww_trigger_e|CMS_trigger_e|g' -e 's|CMS_xww_trigger_m|CMS_trigger_m|g' -e 's|CMS_xww_XS_STop|CMS_XS_STop|g' < ${LVJ8DIR}/${COMBLVJWW8CARD} &> $OUTDIR/${COMBLVJWW8CARD}   
...
if [ $MASS -gt 800 ]
    ...
    sed -e 's|CMS_xww_btag_eff_13TeV|CMS_eff_b_13TeV|g' -e 's|CMS_xww_trigger_e_13TeV|CMS_trigger_e_13TeV|g' -e 's|CMS_xww_trigger_m_13TeV|CMS_trigger_m_13TeV|g' -e 's|CMS_xww_XS_STop_13TeV|CMS_XS_STop_13TeV|g' -e 's|CMS_xww_XS_VV_13TeV|CMS_XS_VV_13TeV|g' < ${LOWMASSLVJ13DIR}/${LOWMASSLVJ13CARDS} &> $OUTDIR/${COMBLVJWW13CARD}
...
LABEL="jjwwzh13"
JJWWZH13CARD="CMS_jj_Zprimefix_WW_ZH_${MASS}_13TeV_CMS_jj_VV.txt"
...
LABEL="lljnnjzh13"
LLJZH13CARD="XZh_M${MASS}.txt"
COMBLLJZH13CARD="comb_${LABEL}.${MASS}.txt"

if [ $MASS -ge 800 ]
    then
    #python adapt_xsec_llj_nnj_ZH_13TeV.py ${MASS}
    sed -e 's|workspaces/||g' -e 's|QCD_scale |CMS_XS_qq_scale |g' -e 's|CMS_scale_pu_13TeV|CMS_pu_13TeV|g' -e 's|pdf_xsec|CMS_XS_qq_PDF|g' -e 's|CMS_xsec_VV|CMS_XS_VV_13TeV|g'  < ${LLJZH13DIR}/datacards/${LLJZH13CARD} &> $OUTDIR/${COMBLLJZH13CARD}
...
if [ $MASS -lt 800 ] #600-750
    ...
    # 13 TeV LVJ+JJ+LLBB+NNBB
    combineCards.py $COMBLVJWW13CARD &> $COMBALLZPRIME13
            
    # 8 TeV LLLV+LLJ+LVJ+JJ (WV)
    combineCards.py $COMBLLJZH8CARD &> $COMBALLWVZPRIME8

    #8 + 13 TeV LLLV+LLJ+LVJ+JJ (WV)
    combineCards.py $COMBLLJZH8CARD $COMBLVJWW13CARD &> $COMBALLWVZPRIME138

    #8 TeV VH+WV
    combineCards.py $COMBLLJZH8CARD &> $COMBALLZPRIME8

    #8 + 13 TeV VH+WV
    combineCards.py $COMBLLJZH8CARD $COMBLVJWW13CARD &> $COMBALLZPRIME138
